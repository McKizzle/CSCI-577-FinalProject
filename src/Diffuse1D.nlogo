...
to setup-road
  ...
  if (pycor = 0) and (pxcor = min-pxcor) [ ; End of a road is a source.
...
;; Resets sinks and sources to their globally defined values. 
to reset-sinks-sources
  
end

;; Applies diffusion to the path tiles. So that the turtles can travel from the 
;; source to the sink. If a neighboring tile is not part of the path then do not
;; apply any form of diffusion to it. 
to diffuse-sink-dist
  ask patches [patches-diffuse]
end
